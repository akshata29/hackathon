...
class BaseOrchestrator(ABC):
    ...
    def build_specialist_agents(self, user_token: str | None = None, raw_token: str | None = None) -> list:
        ...
        Args:
            user_token:  Stable user identifier (email / oid) for session routing.
            raw_token:   Raw Entra Bearer token string; pass to build_obo_http_client
                         when constructing MCP-backed tools for private data access.
        """
        ...
    ...
    def build_concurrent_agents(self, user_token: str | None = None, raw_token: str | None = None) -> list:
        ...
        return self.build_specialist_agents(user_token, raw_token=raw_token)
    ...
    def _build_handoff_workflow(self, user_token: str | None = None, raw_token: str | None = None):
        ...
        specialists = self.build_specialist_agents(user_token, raw_token=raw_token)
    ...
    def _build_concurrent_workflow(self, user_token: str | None = None, raw_token: str | None = None):
        ...
        specialists = self.build_concurrent_agents(user_token, raw_token=raw_token)
    ...
    async def run_handoff(
        ...
        if self.comprehensive_trigger and self.comprehensive_trigger in triage_text:
            ...
            async for event in self.run_comprehensive(message, session_id, user_token, raw_token=raw_token, history=history):
                yield event
        else:
            for buffered in triage_buffer:
                yield buffered
