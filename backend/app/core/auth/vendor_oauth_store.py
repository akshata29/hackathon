...
class VendorOAuthStore:
    ...
    async def retrieve_token(self, user_oid: str) -> str | None:
        ...
        if self._container is None:
            await self.initialize()
        doc_id = f"{user_oid}-{self._vendor}"
        try:
            ...
            logger.debug("retrieve_token: doc_id=%r found=%s", doc_id, bool(token))
            return token
        except Exception as exc:
            logger.warning("retrieve_token: doc_id=%r not found (%s: %s)", doc_id, type(exc).__name__, exc)
            return None
